...
def build_single_relationship(
    ...
    logger.info(f'Building relationship for parent class to child ({child_cls}) {relation_def}')
    return relationship(
        child_cls,
...
def get_parent_relationship_def(parent_table_name: TableNames, child_table_name: TableNames) -> ParentRelation:
    rd = RELATIONSHIPS[parent_table_name][child_table_name]
    logger.info(f'Got relationship definition from {parent_table_name} to {child_table_name}: {rd}')
    return rd

def get_relationship(parent_table_name: TableNames, child_table_name: TableNames) -> Mapped:
    logger.info(f'Getting relationship from {parent_table_name} to {child_table_name}')
